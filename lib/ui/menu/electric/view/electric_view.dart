...
class ElectricView extends StatefulWidget {

  ElectricView({ Key? key }) : super(key: key);

  @override
  State<ElectricView> createState() => _ElectricViewState();
}

class _ElectricViewState extends State<ElectricView> {
                      ...
                      if (state is ElectricLoading) {
                        ...
                        return Expanded(
                          child: TabBarView(
                            children: [
                              prepaid(state),
                              postpaid(context, state.listRecent)
                            ]
                          ),
                        );
                      } else {
  ...
  Widget prepaid(ElectricLoaded electricLoaded){
    return ListView(
      children: [
        Card(
        ...
        const SizedBox(height: 10,),
        listToken(electricLoaded.electricTokenResponse),
        const SizedBox(height: 20,),
        Component.textBold(
          "Transaksi Terakhir",
        ...
        Component.divider(),
        recent(electricLoaded.listRecent, true)
      ],
    );
  }

  Widget postpaid(context, List<RecentNumberResponse> listRecent){
    return ListView(
      children: [
        Card(
          margin: const EdgeInsets.only(bottom: 10),
        ...
        const SizedBox(height: 8,),
        Component.divider(),
        recent(listRecent, false)
      ],
    );
  }

  Widget recent(List<RecentNumberResponse> listRecent, bool isPrepaid){
        ...
        itemBuilder: (BuildContext context, int index) {
          return InkWell(
            onTap: () {
              if(isPrepaid) {
                setState(() {
                  idContactController.text = listRecent[index].numbers ?? "";
                });
              } else {
                BlocProvider.of<ElectricCubit>(context).onPostpaidInquiry(listRecent[index].numbers ?? "");
              }
            },
            child: Card(
              ...
              child: Padding(
                ...
                child: Row(
                  children: [                
                    Image.asset( 
                      ViewUsecase.iconFeature(Feature.electric),
                      height: 40,
                    ),
